...
struct CalibrationData//strukture for calibration and measurement values
{
  short AC1;
  short AC2;
  short AC3;
...
struct CalibrationData getCalibration(struct CalibrationData cal)
{
  uint8_t i = 0xaa;
  uint8_t j = 0;
  for(j=0;j<11;j++)
...
struct CalibrationData getTemp(struct CalibrationData cal)
{
  i2c_start(0xee);
  i2c_write(0xf4);
  i2c_write(0x2e);
...
struct CalibrationData getPressure(struct CalibrationData cal)
{
  i2c_start(0xee);
  i2c_write(0xf4);
  i2c_write(0x34);
